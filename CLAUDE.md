...
## Демонстрационный ноутбук (auto_git_review_research)
...
напрямую): промпты встроены в словарь `PROMPTS`, 41 unit-тест — в раздел 10.

Вместе с ноутбуком в папку `auto_git_review_research/` для подачи на валидацию
скопированы `README.md`, `ответ_на_замечания_валидации.md` (+ `.pdf`), исходный
отчёт по валидации (`[Номер ЗНО C-1101243] Отчет по валидации.pdf`), презентация
(`Автоматический_ИИ_ревью_Pull_Request_v1.pptx`), `.env.example` и
`requirements.txt` — полный комплект материалов для рецензента. Отдельные файлы
`prompts/`, `tests/`, `pytest.ini` и `requirements-dev.txt` в папку подачи **не
выносятся** — они встроены в сам ноутбук.
...
корпоративные значения в демо-материалы не добавлять.

`similarity_search/` — исследовательская папка другого автора, к проекту не
относится и **в git не коммитится**.
