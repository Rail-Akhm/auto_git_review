...
│   ├── auto_git_review_research.ipynb   # Самодокументирующий ноутбук-разбор (для демонстрации)
...
разбор проекта для демонстрации внешней аудитории (комиссия, другие команды).
Весь код пайплайна (`config.py`, `alm.py`, `llm.py`, `prompt.py`, `review.py`)
встроен в клетки дословно; инварианты H1–H4 (dedup-маркер, лимиты батчей,
агрегация вердиктов, защита `MAX_BATCHES`) проверены на синтетических
сценариях. Живой раздел ходит в реальные ALM/LLM только при наличии `.env` и
корпоративной сети (комментарии в PR ноутбук не постит).
